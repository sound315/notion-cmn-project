...
## Phase 8 — 방문자 문의 폼 (Contact Form)

**배경**: 블로그 방문자가 이름/이메일/메시지를 입력하면 Notion Contacts DB에 저장되고, 관리자 대시보드에서 확인할 수 있다.
...
#### Notion DB
- [ ] Contacts DB 생성 (이름, 이메일, 메시지, 읽음 여부, 문의일)
- [ ] `NOTION_CONTACTS_DATABASE_ID` 환경변수 추가

#### 타입 정의 (`types/contact.ts`)
- [ ] `Contact` 인터페이스

#### Notion API 클라이언트 (`lib/contact.ts`)
- [ ] `createContact()` — 문의 저장
- [ ] `getContacts()` — 문의 목록 조회 (관리자용)
- [ ] `markAsRead()` — 읽음 처리

#### API 라우트
- [ ] `POST /api/contact` — 문의 제출 (공개)
- [ ] `PATCH /api/admin/contacts/[id]` — 읽음 처리 (관리자)

#### 페이지 및 컴포넌트
- [ ] `/contact` 페이지 — 문의 폼
- [ ] `/admin/contacts` 페이지 — 문의 목록 (읽음/안읽음 구분)
- [ ] 헤더에 '문의' 메뉴 추가

### 완료 기준

- 방문자가 문의를 제출하면 Notion DB에 저장된다
- 관리자가 `/admin/contacts`에서 문의 목록을 확인하고 읽음 처리할 수 있다
- 중복 제출 방지(로딩 상태)와 성공 메시지가 표시된다
